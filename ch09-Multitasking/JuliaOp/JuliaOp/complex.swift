...
import Foundation
protocol RealType : FloatingPoint, AbsoluteValuable, Equatable, Comparable, Hashable {
    // Initializers
    init(_ value: UInt8)
    init(_ value: Int8)
    init(_ value: UInt16)
...
// atan2(z, zz)
func atan2<T>(_ z:Complex<T>, _ zz:Complex<T>) -> Complex<T> {
    return atan(z / zz)
}
// asin(z)
